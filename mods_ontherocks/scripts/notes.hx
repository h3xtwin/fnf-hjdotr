...
function onGameOver() {
    if (PlayState.SONG.song != 'asa') {
        CustomSubstate.openCustomSubstate('g', true);
    }
    else{
        CustomSubstate.openCustomSubstate('o', true);
    }
...
function onCustomSubstateCreate(name)
{
    ...
    cooked = new FlxSprite(0,0,Paths.image('opeor/cookedyo'));
    cooked.screenCenter();
    cooked.alpha = 0;
    add(cooked);
...
function killyourself(){
    ...
    cooked.alpha = 1;
    //FlxG.sound.playMusic(Paths.music('gameOver'), 6);
    FlxG.sound.playMusic(Paths.music('seventynine'), 6);



    new FlxTimer().start(18.5, (tmr:FlxTimer) -> {
        PauseSubState.restartSong();
    });
